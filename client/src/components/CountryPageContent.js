...
export default function CountryPageContent() {
  ...
  const currentPhoto = useSelector((state) => state.countriesStore.currentCountry.photo);
  const currentGallery = useSelector((state) => state.countriesStore.currentCountry.gallery);
  ...
  return (
    <div>
      ...
      <h3>{currentInfo}</h3>
      <h4>{country.name}</h4>
      <img src={currentPhoto} alt="Somebody" />
